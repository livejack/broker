...
const clientSource = require('uglify-js').minify(
	readSync(require.resolve('socket.io-client/dist/socket.io.slim.js')).toString()
).code;
const clientSourceGz = zlib.gzipSync(clientSource, {level: zlib.Z_BEST_COMPRESSION});
const clientSourceDfl = zlib.deflateSync(clientSource, {level: zlib.Z_BEST_COMPRESSION});

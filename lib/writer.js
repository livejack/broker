...
exports.master = function(nsp, namespace, config) {
	...
	var nsconfig = config.namespaces[namespace];
	nsp.on('connection', function(socket) {
		...
		socket.on('message', function(msg) {
			...
			if (!msg.iotime) {
				...
				msg.iotime = (new Date()).toISOString();
			}
...
exports.slave = function(nsp, namespace, config) {
	...
	var nsconfig = config.namespaces[namespace];
	var client = ioclient(config.servers[config.server][0].href + namespace);
	...
	client.on('error', function(err) {
		if (err) console.error("Error in client of", master, err);
	});
...
exports.post = function(spaces) {
	return function(req, res, next) {
		...
		var nsp = spaces[namespace];
		if (!nsp) {
			return res.sendStatus(503);
		}
		if (!msg.key) {
			return res.status(400).send("Message received without key", msg);
		}

		var parents = msg.parents || [];
		var room = parents.slice(-1).pop() || msg.key;
...
function getRoom(msg) {
	var room = msg.room;
	// for compatibility with raja, add parents only if msg.room is not set
	if (!room) {
		if (!msg.parents) msg.parents = [];
		room = msg.parents && msg.parents.slice(-1).pop() || msg.key;
	}
	return room;
}
...
function initScopes(socket, bearer, nsconfig, room, cb) {
	...
	jwt.verify(bearer, nsconfig.publicKey, {
		...
		if (!scopes) return cb();
		var spocks = socket.request.scopes;
		if (!spocks) spocks = socket.request.scopes = {};
		var rscopes = spocks[room] || {};
		// handle expiration
		var expiration = parseInt(obj.exp);
		if (!isNaN(expiration)) { // do not handle expiration for now
			if (rscopes.deletion) {
				clearTimeout(rscopes.deletion);
				delete rscopes.deletion;
			}
			var delay = expiration * 1000 - Date.now();
...
function cleanScopes(socket, room) {

}

function convertScopes(input) {
	var scopes = {};
	if (Array.isArray(input)) {
		input.forEach(function(scope) {
			scopes[scope] = {read: true};
		});
	} else {
		scopes = input;
	}
